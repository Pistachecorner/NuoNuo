...
function errText(e){const m=e?.message||e?.details||e?.hint||'Unknown Supabase error.';if(/rate limit|too many emails|over_email_send_rate_limit/i.test(m))return 'NuoNuo email service is temporarily rate-limited by Supabase. Wait for the limit to reset, or enable Custom SMTP / turn off Confirm email in Supabase Auth for testing.';return m}
