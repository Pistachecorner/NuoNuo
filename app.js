...
async function enforceManagementAccess(user, p){
  // Customer accounts must NEVER enter Management, even if an old/incorrect profile row still exists.
  const accountType=String(user?.user_metadata?.account_type||"").toLowerCase();
  if(accountType === "customer"){
    console.warn("Management access denied for customer account:", user?.email);
    try{ await sb.auth.signOut({scope:"local"}); }catch(_){}
    if(NUONUO_SITE_URL){ window.location.replace(NUONUO_SITE_URL); return false; }
    throw new Error("This is a NuoNuo customer account. Management access is not allowed.");
  }
