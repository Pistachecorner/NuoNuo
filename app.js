...
function addonOptionsForProduct(productId){const product=products.find(p=>String(p.id)===String(productId));const category=cats.find(c=>sameId(c.id,product?.category_id));const categoryName=String(category?.name||'').trim().replace(/\s+/g,' ').toLowerCase();const addonCategories=new Set(['dubai chewy cookies (marshmellow version)','dubai chewy cookies (mochi version)']);if(!addonCategories.has(categoryName))return [];const ids=productAddonLinks.filter(x=>String(x.product_id)===String(productId)).map(x=>String(x.addon_id));return ids.map(id=>addons.find(a=>String(a.id)===id)).filter(Boolean)}
