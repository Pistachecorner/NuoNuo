...
function renderHero(){
  ...
  if(!holder)return;
  // Keep the original Hero artwork independent from Popular Products.
  // Do not source the Hero image from the live Management menu.
  const heroImage='hero/nuonuo-hero-matcha-reserve.png';
  holder.innerHTML=`<img class="hero-static-image" style="width:100%;height:100%;object-fit:cover;display:block;" src="${heroImage}" alt="Matcha Reserve Dubai Chewy Cookies × Strawberry">${slides.length?`<div class="hero-controls" aria-label="Hero image controls"><button type="button" class="hero-arrow hero-prev" aria-label="Previous">‹</button><div class="hero-dots">${slides.map((_,i)=>`<button type="button" class="hero-dot ${i===heroIndex?'active':''}" data-hero-index="${i}" aria-label="Slide ${i+1}"></button>`).join('')}</div><button type="button" class="hero-arrow hero-next" aria-label="Next">›</button></div>`:''}`;
  if(holder.dataset.bound==='1')return;
  holder.dataset.bound='1';
  holder.addEventListener('click',e=>{
...
async function saveProfile(profile){const {data:{session}}=await sb.auth.getSession();if(!session)throw new Error('Please sign in first.');const normalizedPhone=normalizePhone(profile.phone)||normalizePhone(session.user.phone);const name=profile.name?.trim()||'';const email=profile.email?.trim()||'';const address=profile.address?.trim()||'';const birthday=profile.birthday||null;const payload={auth_user_id:session.user.id,owner_id:owner,name:name||null,phone:normalizedPhone||null,email:email||null,address:address||null,birthday,updated_at:new Date().toISOString()};const {data,error}=await sb.from('nuonuo_customer_profiles').upsert(payload,{onConflict:'auth_user_id,owner_id'}).select().single();if(error)throw error;if(name&&normalizedPhone){const {error:customerError}=await sb.rpc('sync_nuonuo_customer_profile',{p_name:name,p_phone:normalizedPhone,p_email:email||null,p_address:address||null,p_birthday:birthday});if(customerError)throw customerError}currentProfile=data;return data}
