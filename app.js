...
/* =========================================
   NuoNuo night sky v3
   Stars vary in size and twinkle independently.
   One meteor launches every 2 seconds, cycling through
   upper / middle / lower areas of the screen.
   ========================================= */
(function initNuoNuoNightSky(){
  ...
  const stars=[
    [7,14,2],[16,28,4],[27,10,3],[38,22,2],[49,8,4],[61,18,3],
    [73,11,2],[86,24,3],[94,14,4],[11,46,3],[22,61,2],[34,48,4],
    [47,67,3],[58,43,2],[69,58,4],[81,48,3],[92,65,2],[5,82,4],
    [17,91,3],[30,78,2],[43,90,4],[55,77,3],[68,86,2],[79,74,4],
    [90,89,3],[97,78,2],[12,72,2],[25,37,3],[39,58,2],[52,31,3],
    [64,72,2],[76,37,3],[88,53,2],[96,36,3]
  ];

  stars.forEach((pos,i)=>{
    const star=document.createElement('span');
    star.className='nuonuo-star';
    star.style.left=pos[0]+'%';
    star.style.top=pos[1]+'%';
    star.style.setProperty('--star-size',pos[2]+'px');
    star.style.setProperty('--star-opacity',(0.28+(i%5)*0.09).toFixed(2));
    star.style.setProperty('--star-bright',(0.58+(i%4)*0.10).toFixed(2));
    star.style.setProperty('--star-duration',(2.8+(i%6)*0.55).toFixed(2)+'s');
    star.style.setProperty('--star-delay',(-((i*0.83)%7)).toFixed(2)+'s');
  ...
  function launchMeteor(){
    ...
    meteor.style.top=zone.top;
    meteor.style.left=zone.left;
    space.appendChild(meteor);
    meteor.addEventListener('animationend',()=>meteor.remove(),{once:true});
  }

  // First meteor appears shortly after load, then every 2 seconds.
