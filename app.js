...
function parseMeasureUnit(unit){
  ...
  const compound=raw.match(/^([0-9]+(?:\.[0-9]+)?)(kg|g|l|ml|pcs|pc|unit|bottle|bottles|bag|bags|個|件|包|瓶|盒)\/(box|boxes|pack|packs|bottle|bottles|bag|bags|unit|units)$/i);
  if(compound){
    ...
    else if(['bottle','bottles'].includes(baseRaw)) baseUnit='bottle';
    else if(['bag','bags'].includes(baseRaw)) baseUnit='bag';
    if(baseRaw==='unit') baseUnit='unit';
    return {amount,unit:baseUnit,purchaseUnit,valid:true,compound:true};
  }
  const m=raw.match(/([0-9]+(?:\.[0-9]+)?)(kg|g|l|ml|pcs|pc|unit|bottle|bottles|bag|bags|box|boxes|個|件|包|瓶|盒)$/i);
  ...
  if(['bottle','bottles'].includes(u)) return {amount,unit:'bottle',purchaseUnit:'bottle',valid:true};
  if(['bag','bags'].includes(u)) return {amount,unit:'bag',purchaseUnit:'bag',valid:true};
...
function inventoryDisplayUnit(unit){
  const u=String(unit??'').trim().toLowerCase();
  const map={個:'pcs',件:'pcs',包:'pcs',瓶:'bottle',盒:'box',pcs:'pcs',pc:'pcs',unit:'unit',units:'unit',bottle:'bottle',bottles:'bottle',bag:'bag',bags:'bag',box:'box',boxes:'box',pack:'pack',packs:'pack',g:'g',kg:'kg',ml:'ml',l:'l'};
  return map[u]||u||'unit';
}
...
function recipeIngredientUnit(item){
  // Packaging uses its configured base unit (pcs / bottle / bag).
...
const pages = {
  ...
  sales:"Sales", expenses:"Expenses", invoices:"Invoices", purchasing:"Purchasing", suppliers:"Suppliers", movements:"Inventory Movements", audit:"Audit Log", reports:"Reports", partners:"Partners & Profit", financials:"Company Finances", settings:"Profile", staff:"Staff"
};
...
function inventoryActive(item){ return item?.is_active!==false; }
function inventoryStatusState(item){ return inventoryActive(item)?'active':'inactive'; }
function inventoryActiveBadge(item){
  return inventoryActive(item)
    ? '<span class="badge">Active</span>'
    : '<span class="badge" style="background:#f3f3f3;color:#777">Inactive</span>';
}

function renderInventoryTable(){
  const items=window.__inventoryItems||[];
  const filter=window.__inventoryFilter||'ingredient';
  const statusFilter=window.__inventoryStatusFilter||'active';
  ...
  const categoryItems=items.filter(i=>(i.item_type||'other')===filter);
  const filtered=categoryItems.filter(i=>statusFilter==='all' ? true : inventoryStatusState(i)===statusFilter);
  const rows=filtered.map(i=>{
    const purchaseUnit=inventoryUnitSummary(i);
    const stockLabel=inventoryDisplayStock(i);
    const value=inventoryValue(i);
    const active=inventoryActive(i);
    return `<tr${active?'':' style="opacity:.62"'}><td><strong>${esc(i.name)}</strong></td><td>${esc(purchaseUnit)}</td><td>${money(Number(i.cost_per_unit||0))}</td><td>${esc(stockLabel)}</td><td>${money(value)}</td><td>${inventoryStatusBadge(i)} ${inventoryActiveBadge(i)}</td><td><div class="inventory-actions"><button class="btn" type="button" onclick="openInventoryItemModalById('${i.id}')">Edit</button><button class="btn ${active?'btn-danger':''}" type="button" onclick="toggleInventoryActive('${i.id}',${active})">${active?'Set Inactive':'Set Active'}</button></div></td></tr>`;
  }).join('');
  const counts={ingredient:0,packaging:0,kitchenware:0,electronic:0,equipment:0,other:0};
  const activeCounts={ingredient:0,packaging:0,kitchenware:0,electronic:0,equipment:0,other:0};
  const inactiveCounts={ingredient:0,packaging:0,kitchenware:0,electronic:0,equipment:0,other:0};
  items.forEach(i=>{const t=counts[i.item_type]!=null?i.item_type:'other';counts[t]++;inventoryActive(i)?activeCounts[t]++:inactiveCounts[t]++;});
  const activeItems=items.filter(inventoryActive);
  const totalValue=activeItems.reduce((sum,i)=>sum+(inventoryValue(i)),0);
  const categoryValues={ingredient:0,packaging:0,kitchenware:0,electronic:0,equipment:0,other:0};
  activeItems.forEach(i=>{const t=categoryValues[i.item_type]!=null?i.item_type:'other'; categoryValues[t]+=inventoryValue(i);});
  const lowCount=activeItems.filter(i=>Number(i.current_stock||0)<=Number(i.low_stock_threshold||0)).length;
  const tabs=['ingredient','packaging','kitchenware','electronic','equipment','other'].map(k=>`<button type="button" class="inventory-tab ${filter===k?'active':''}" onclick="setInventoryFilter('${k}')">${esc(labels[k])} <span>${statusFilter==='active'?activeCounts[k]:statusFilter==='inactive'?inactiveCounts[k]:counts[k]}</span></button>`).join('');
  const statusBtn=(v,label)=>`<button class="btn ${statusFilter===v?'btn-dark':''}" type="button" onclick="setInventoryStatusFilter('${v}')">${label}</button>`;
  $("#page").innerHTML=`<div class="rpt-head"><div><h3>Inventory</h3><p class="muted">Inventory is separated by category so ingredients, packaging, kitchenware and equipment are easy to manage.</p></div><button class="btn btn-dark" onclick="openInventoryItemModal()">+ Inventory Item</button></div>
    <div class="stats">
      ...
      <div class="stat"><div class="label">Ingredients Value</div><div class="value">${money(categoryValues.ingredient)}</div><div class="muted">${activeCounts.ingredient} active items</div></div>
      <div class="stat"><div class="label">Packaging Value</div><div class="value">${money(categoryValues.packaging)}</div><div class="muted">${activeCounts.packaging} active items</div></div>
      <div class="stat"><div class="label">Kitchenware Value</div><div class="value">${money(categoryValues.kitchenware)}</div><div class="muted">${activeCounts.kitchenware} active items</div></div>
      <div class="stat"><div class="label">Electronic Equipment Value</div><div class="value">${money(categoryValues.electronic)}</div><div class="muted">${activeCounts.electronic} active items</div></div>
      <div class="stat"><div class="label">Equipment Value</div><div class="value">${money(categoryValues.equipment)}</div><div class="muted">${activeCounts.equipment} active items</div></div>
      <div class="stat"><div class="label">Other Value</div><div class="value">${money(categoryValues.other)}</div><div class="muted">${activeCounts.other} active items</div></div>
      <div class="stat"><div class="label">Low Stock</div><div class="value">${lowCount}</div></div>
    </div>
    <div class="inventory-tabs">${tabs}</div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin:12px 0">${statusBtn('active','Active')} ${statusBtn('inactive','Inactive')} ${statusBtn('all','All')}</div>
    <div class="card"><div class="rpt-head" style="margin:0 0 14px;padding:0;border:0"><div><h4 style="margin:0">${esc(labels[filter])}</h4><p class="muted" style="margin:5px 0 0">${filtered.length} item${filtered.length===1?'':'s'} shown · ${activeCounts[filter]} active · ${inactiveCounts[filter]} inactive.</p></div></div><div class="table-wrap"><table><thead><tr><th>Name</th><th>Purchase Unit</th><th>Cost / Unit</th><th>Current Stock</th><th>Current Value</th><th>Stock / Status</th><th>Actions</th></tr></thead><tbody>${rows||`<tr><td colspan="7" class="empty">No ${esc(labels[filter]).toLowerCase()} in this status.</td></tr>`}</tbody></table></div></div>`;
  installPageSearch("inventory");
}
function setInventoryStatusFilter(filter){window.__inventoryStatusFilter=filter||'active';renderInventoryTable();}

async function toggleInventoryActive(id,currentlyActive){
  if(!isOwner())return toast('Only the owner can change inventory item status.');
  const next=!currentlyActive;
  const {data:item,error:loadError}=await sb.from('ingredients').select('id,name,is_active').eq('id',id).eq('user_id',dataUserId()).single();
  if(loadError)return toast(errText(loadError));
  if(!confirm(`${next?'Activate':'Set inactive'} inventory item "${item.name}"? ${next?'It will appear in active inventory and be available for new recipe/add-on/purchase entries.':'It will remain in historical records and can be restored later, but will be hidden from active inventory and unavailable for new recipe/add-on/purchase entries.'}`))return;
  const {error}=await sb.from('ingredients').update({is_active:next}).eq('id',id).eq('user_id',dataUserId());
  if(error)return toast(errText(error));
  toast(next?'Inventory item activated.':'Inventory item set to inactive.');
...
const render = {
  async dashboard(){
    ...
    const [{data:todaySalesRows,error:toError},{data:pendingOrders,error:poError},{data:inventory,error:invError}]=await Promise.all([
      ...
      sb.from("ingredients").select("*").eq("user_id",dataUserId()).eq("is_active",true).order("item_type",{ascending:true}).order("name")
    ]);
  ...
  async inventory(){
    ...
    window.__inventoryStatusFilter=window.__inventoryStatusFilter||'active';
  ...
  async orders(){
    const {data:orderRows,error}=await sb.from("orders").select("*,customers(name)").eq("user_id",dataUserId()).eq("sales_channel",activeChannel);
    if(error)throw error;

    // ORDERS page: Date + sorting come from PAYMENT (sales.sale_date),
    // never from order_date / scheduled_date. Pending Orders has its own
    // schedule-date logic and is intentionally untouched.
    let salesRows=[];
    let salesHasCreatedAt=true;
    let salesQuery=await sb.from("sales")
      .select("id,order_id,sale_date,created_at,sales_channel,amount,payment_method")
      .eq("user_id",dataUserId())
      .order("sale_date",{ascending:false});

    if(salesQuery.error){
      const msg=String(salesQuery.error.message||"").toLowerCase();
      if(msg.includes("created_at")&&(msg.includes("column")||msg.includes("schema cache"))){
        salesHasCreatedAt=false;
        salesQuery=await sb.from("sales")
          .select("id,order_id,sale_date,sales_channel,amount,payment_method")
          .eq("user_id",dataUserId())
          .order("sale_date",{ascending:false});
      }
    }
    if(salesQuery.error)throw salesQuery.error;
    salesRows=salesQuery.data||[];

    // Resolve the sales channel for older sales rows that do not have one.
    const linkedOrderIds=[...new Set(salesRows.map(s=>s.order_id).filter(Boolean))];
    let orderChannelMap=Object.create(null);
    if(linkedOrderIds.length){
      const {data:linkedOrders,error:linkedError}=await sb.from("orders")
        .select("id,sales_channel")
        .eq("user_id",dataUserId())
        .in("id",linkedOrderIds);
      if(linkedError)throw linkedError;
      orderChannelMap=Object.fromEntries((linkedOrders||[]).map(o=>[o.id,o.sales_channel||"nuonuo"]));
    }
    salesRows=salesRows.filter(s=>(s.sales_channel||orderChannelMap[s.order_id]||"nuonuo")===activeChannel);

    const paymentByOrder=Object.create(null);
    const usedSales=new Set();

    // First choice: the real order_id relation.
    for(const sale of salesRows){
      if(!sale.order_id)continue;
      const key=sale.id||`${sale.order_id}|${sale.sale_date}`;
      const previous=paymentByOrder[sale.order_id];
      const saleDate=String(sale.sale_date||"").slice(0,10);
      const saleTime=salesHasCreatedAt?String(sale.created_at||""):"";
      const previousKey=previous ? `${previous.saleDate}|${previous.saleTime}` : "";
      const currentKey=`${saleDate}|${saleTime}`;
      if(!previous || currentKey>previousKey){
        paymentByOrder[sale.order_id]={saleDate,saleTime};
      }
      usedSales.add(key);
    }

    // Legacy fallback: some old sales rows may have no order_id. Match them
    // to a paid order by payment date + amount + payment method. This is only
    // for display/sorting; it never changes database data.
    const paidOrders=(orderRows||[]).filter(o=>String(o.payment_status||o.payment||"").toLowerCase()==="paid");
    for(const sale of salesRows){
      if(sale.order_id)continue;
      const saleDate=String(sale.sale_date||"").slice(0,10);
      const amount=Number(sale.amount||0);
      const method=String(sale.payment_method||"").trim().toLowerCase();
      const candidates=paidOrders.filter(o=>{
        if(paymentByOrder[o.id])return false;
        if(Math.abs(Number(o.total||0)-amount)>0.0001)return false;
        const om=String(o.payment_method||"").trim().toLowerCase();
        if(method && om && method!==om)return false;
        return true;
      });
      if(candidates.length===1){
        paymentByOrder[candidates[0].id]={saleDate,saleTime:salesHasCreatedAt?String(sale.created_at||""):""};
      }
    }
    ...
    const orders=(orderRows||[]).map(o=>{
      ...
      return {
        ...o,
        payment_date:String(payment.saleDate||""),
        payment_time:String(payment.saleTime||"")
      };
    });

    const paymentSortKey=o=>{
      const status=String(o.payment_status||o.payment||"").toLowerCase();
      if(status!=="paid")return -1;
      const date=String(o.payment_date||"");
      const time=String(o.payment_time||"");
      if(!date)return 0;
      const iso=time || `${date}T00:00:00+08:00`;
      const t=Date.parse(iso);
      return Number.isNaN(t)?Date.parse(`${date}T00:00:00+08:00`):t;
    };

    // Newest payment first. Unpaid orders are always below paid orders.
    orders.sort((a,b)=>{
      const as=String(a.payment_status||a.payment||"").toLowerCase()==="paid";
      const bs=String(b.payment_status||b.payment||"").toLowerCase()==="paid";
      if(as!==bs)return as?-1:1;
      const at=paymentSortKey(a),bt=paymentSortKey(b);
      if(at!==bt)return bt-at;
      if(String(a.payment_date||"")!==String(b.payment_date||""))return String(b.payment_date||"").localeCompare(String(a.payment_date||""));
...
function orderCreatedBusinessDate(order){
  const raw=String(order?.created_at||"");
  if(!raw) return "Date not set";
  const d=new Date(raw);
  if(Number.isNaN(d.getTime())) return "Date not set";
  return new Intl.DateTimeFormat("en-CA",{timeZone:SHOP_TIMEZONE,year:"numeric",month:"2-digit",day:"2-digit"}).format(d);
}
...
function orderTable(data,full=false,pending=false,itemCounts={},remainingByOrder={}){
  const rows=(data||[]).map(o=>{
    ...
    const type=orderTypeOf(o), scheduleDate=orderScheduleDate(o), displayDate=pending ? (scheduleDate||"Date not set") : (o.payment_date||"Not paid");
...
async function openInventoryItemModal(options={}){
  ...
  const unit=item?.unit||(ingredientType?'1000g/unit':defaultType==='packaging'?'1pcs/box':'unit');
  ...
  const packagingPurchaseUnits=[['box','Box'],['pack','Pack'],['bottle','Bottle'],['bag','Bag'],['unit','Unit']];
  const physicalPurchaseUnits=[['pcs','Pcs'],['unit','Unit']];
  let purchaseAmount=1, purchaseUnit=ingredientType?'unit':(defaultType==='packaging'?'box':'unit'), containsAmount=packAmount, containsUnit=baseUnit;
  if(defaultType==='packaging' && !parsed.compound && !parsed.valid){ containsAmount=1; containsUnit='pcs'; }
  if(parsed.compound){
  ...
  const whole=['pcs','unit','box','pack','bottle','bag'].includes(baseUnit);
  const unitSummary=ingredientType
    ...
    : (defaultType==='packaging'?`${Number(parsed.amount||1).toLocaleString()} ${parsed.unit||baseUnit}/${parsed.purchaseUnit||'box'}`:`${Number(parsed.amount||1).toLocaleString()} ${parsed.unit||baseUnit}`);
  const editPurchaseUnit=ingredientType ? (parsed.purchaseUnit&&['box','unit','pack','bottle'].includes(parsed.purchaseUnit)?parsed.purchaseUnit:'unit') : (defaultType==='packaging'?(parsed.purchaseUnit&&['box','pack','bottle','bag','unit'].includes(parsed.purchaseUnit)?parsed.purchaseUnit:'box'):'unit');
  const editContainsAmount=(ingredientType||defaultType==='packaging') ? Number(parsed.amount||1) : 1;
  const editContainsUnit=(ingredientType||defaultType==='packaging') ? (parsed.unit||baseUnit||(defaultType==='packaging'?'pcs':'g')) : baseUnit;
  ...
  const body=`<form id="inventoryItemForm" class="form-grid">
    ...
    ${item ? ((ingredientType || defaultType==='packaging') ? `<div class="wide notice" style="display:block!important"><strong>Purchase unit definition</strong><div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(180px,220px);gap:12px;margin-top:10px"><label>Quantity per Purchase Unit<div style="display:flex;gap:8px"><input id="invEditContainsAmount" type="number" min="0.000001" step="0.001" value="${editContainsAmount}" oninput="updateInventoryItemFields()"><select id="invEditContainsUnit" onchange="updateInventoryItemFields()">${ingredientType?`<option value="g" ${editContainsUnit==='g'?'selected':''}>g</option><option value="kg" ${editContainsUnit==='kg'?'selected':''}>kg</option><option value="ml" ${editContainsUnit==='ml'?'selected':''}>ml</option><option value="pcs" ${editContainsUnit==='pcs'?'selected':''}>pcs</option><option value="unit" ${editContainsUnit==='unit'?'selected':''}>unit</option>`:`<option value="pcs" ${editContainsUnit==='pcs'?'selected':''}>pcs</option><option value="bottle" ${editContainsUnit==='bottle'?'selected':''}>bottle</option><option value="bag" ${editContainsUnit==='bag'?'selected':''}>bag</option>`}</select></div></label><label>Purchase Unit<select id="invEditPurchaseUnit" onchange="updateInventoryItemFields()">${(ingredientType?ingredientPurchaseUnits:packagingPurchaseUnits).map(([v,l])=>`<option value="${v}" ${editPurchaseUnit===v?'selected':''}>${l}</option>`).join('')}</select></label></div><div id="invEditUnitPreview" class="muted small" style="margin-top:8px"></div></div>` : '') : `<label id="invUnitWrap">Purchase Quantity<div style="display:flex;gap:8px;align-items:center"><input id="invUnitAmount" type="number" min="0" step="1" value="0" oninput="updateInventoryItemFields()" style="flex:1"><select id="invUnitType" onchange="updateInventoryItemFields()" style="width:120px">${(ingredientType?ingredientPurchaseUnits:defaultType==='packaging'?packagingPurchaseUnits:physicalPurchaseUnits).map(([v,l])=>`<option value="${v}" ${purchaseUnit===v?'selected':''}>${l}</option>`).join('')}</select></div><div class="muted small" style="margin-top:6px">Enter how many purchase units you are adding. Example: 2 Unit.</div></label>
    <label id="invContainsWrap">Quantity per Purchase Unit<div style="display:flex;gap:8px;align-items:center"><input id="invContainsAmount" type="number" min="0.000001" step="0.001" value="${containsAmount}" oninput="updateInventoryItemFields()" style="flex:1"><select id="invContainsUnit" onchange="updateInventoryItemFields()" style="width:120px">${ingredientType?`<option value="g" ${containsUnit==='g'?'selected':''}>g</option><option value="kg" ${containsUnit==='kg'?'selected':''}>kg</option><option value="ml" ${containsUnit==='ml'?'selected':''}>ml</option><option value="pcs" ${containsUnit==='pcs'?'selected':''}>pcs</option><option value="unit" ${containsUnit==='unit'?'selected':''}>unit</option>`:`<option value="pcs" ${containsUnit==='pcs'?'selected':''}>pcs</option><option value="bottle" ${containsUnit==='bottle'?'selected':''}>bottle</option><option value="bag" ${containsUnit==='bag'?'selected':''}>bag</option>`}</select></div><div class="muted small" style="margin-top:6px">Example: 1 Unit contains 1000 g.</div></label>`}
    ...
    <label>Current Stock <span id="invStockUnit">(${esc(inventoryDisplayUnit(baseUnit))})</span><input id="invStock" type="number" min="0" step="${whole?'1':'0.01'}" value="${currentBase.toFixed(whole?0:2)}"></label>
    ${item?'':`<label id="invAddStockWrap">Add Purchase Quantity <span id="invAddStockUnit">(${esc(inventoryDisplayUnit(baseUnit))})</span><input id="invAddStock" type="number" min="0" step="${whole?'1':'0.01'}" value="0" readonly></label>`}
    <label>Low Stock Threshold <span id="invLowUnit">(${esc(inventoryDisplayUnit(baseUnit))})</span><input id="invLow" type="number" min="0" step="${whole?'1':'0.01'}" value="${lowBase.toFixed(whole?0:2)}"></label>
    <label>Supplier<input id="invSupplier" value="${esc(item?.supplier||'')}"></label>
    <label>Status<select id="invStatus"><option value="true" ${inventoryActive(item)?'selected':''}>Active</option><option value="false" ${!inventoryActive(item)?'selected':''}>Inactive</option></select></label>
    ${item?`<div class="wide muted small">Current Stock is editable directly in ${esc(inventoryDisplayUnit(baseUnit))}. Saving this form does not add or remove a purchase.</div>`:`<div id="invUnitPreview" class="wide notice"></div>`}
  </form>`;
  openModal(item?'Edit Inventory Item':'Add Inventory Item',body,async()=>{
    ...
    if(item){
      ...
      let newPackAmount=1;
      if(existingIngredient || item.item_type==='packaging'){
        const amount=Math.max(Number($('#invEditContainsAmount')?.value||0),0.000001);
        const rawUnit=String($('#invEditContainsUnit')?.value||(item.item_type==='packaging'?'pcs':'g'));
        const purchase=String($('#invEditPurchaseUnit')?.value||(item.item_type==='packaging'?'box':'unit'));
      ...
      const payload={
        ...
        is_active:$('#invStatus').value!=='false'
      };
      ...
      if(options.returnToPurchase){
        const {data:ings,error:ie}=await sb.from('ingredients').select('id,name,unit,cost_per_unit,item_type,current_stock,is_active').eq('user_id',dataUserId()).eq('is_active',true).order('name');
    ...
    if(type==='ingredient'){
      ...
      const allowedBase=['pcs','bottle','bag'];
      const actualBase=allowedBase.includes(containsUnit)?containsUnit:'pcs';
      const allowedPurchase=['box','pack','bottle','bag','unit'];
      const actualPurchase=allowedPurchase.includes(purchaseUnit)?purchaseUnit:'box';
      saveUnit=`${containsAmount}${actualBase}/${actualPurchase}`;
      purchaseBaseAmount=containsAmount;
    }else{
      const allowedPhysical=['pcs','unit'];
    ...
    const payload={user_id:dataUserId(),name,item_type:type,unit:saveUnit,cost_per_unit:Number($('#invCost').value||0),current_stock:Number(storedStock.toFixed(6)),low_stock_threshold:Number(low.toFixed(6)),supplier:$('#invSupplier').value.trim()||'',is_active:$('#invStatus').value!=='false'};
...
function updateInventoryItemFields(){
  ...
  const allowed=isIng?['box','unit','pack','bottle']:(type==='packaging'?['box','pack','bottle','bag','unit']:['pcs','unit']);
  if(unitTypeEl){
  ...
  if(isEdit && (isIng || type==='packaging')){
    const editUnit=String($('#invEditContainsUnit')?.value||(type==='packaging'?'pcs':'g'));
    if(stockUnit)stockUnit.textContent=`(${inventoryDisplayUnit(editUnit)})`;
    if(lowUnit)lowUnit.textContent=`(${inventoryDisplayUnit(editUnit)})`;
    if(stock)stock.step=['pcs','unit','bottle','bag'].includes(editUnit)?'1':'0.01';
    if(low)low.step=['pcs','unit','bottle','bag'].includes(editUnit)?'1':'0.01';
    const editPreview=$('#invEditUnitPreview');
    const editAmount=Math.max(Number($('#invEditContainsAmount')?.value||1),0.000001);
    const editPurchase=String($('#invEditPurchaseUnit')?.value||'box');
    if(editPreview)editPreview.innerHTML=`<strong>1 ${esc(editPurchase)} = ${Number(editAmount).toLocaleString()} ${esc(inventoryDisplayUnit(editUnit))}</strong>`;
    return;
  }
  ...
  if(type==='packaging'){
    ...
    const containsRaw=Math.max(Number(containsAmount?.value||0),1);
    const containsUnit=String(containsUnitEl?.value||'pcs');
    const selectedPurchase=String(unitTypeEl?.value||'box');
    if(hiddenUnit)hiddenUnit.value=`${containsRaw}${containsUnit}/${selectedPurchase}`;
    if(stockUnit)stockUnit.textContent=`(${containsUnit})`;
    if(lowUnit)lowUnit.textContent=`(${containsUnit})`;
    if(addStockUnit)addStockUnit.textContent=`(${containsUnit})`;
    if(stock)stock.step='1'; if(low)low.step='1';
    if(addStock)addStock.value=String(purchaseQty*containsRaw);
    if(preview)preview.innerHTML=isEdit
      ? `<strong>1 ${esc(selectedPurchase)} = ${Number(containsRaw).toLocaleString()} ${esc(containsUnit)}</strong> · Current stock is editable directly in ${esc(containsUnit)}.`
      : `<strong>1 ${esc(selectedPurchase)} = ${Number(containsRaw).toLocaleString()} ${esc(containsUnit)}</strong> · Adding ${purchaseQty} ${esc(selectedPurchase)} = <strong>${Number(purchaseQty*containsRaw).toLocaleString()} ${esc(containsUnit)}</strong>.`;
    return;
  }
  if(containsWrap)containsWrap.hidden=false;
  ...
  const whole=['pcs','unit','box','pack','bottle','bag'].includes(baseUnit); if(stock)stock.step=whole?'1':'0.01'; if(low)low.step=whole?'1':'0.01';
...
async function openPackagingModal(item=null){
  openModal(item?'Edit Packaging':'Add Packaging',`<form id="packagingForm" class="form-grid">
    ...
    <label>Status<select id="pStatus"><option value="true" ${inventoryActive(item)?'selected':''}>Active</option><option value="false" ${!inventoryActive(item)?'selected':''}>Inactive</option></select></label>
